...
### Приведение типов
- при передачи аргумента типа, который входит в диапазон типа параметра, происходит автоматическое (неявное) приведение типа
...
- явное приведение
  ...
  - НО! если при явном значение больше чем диапазон меньшего, будет непредсказуемый результат
...
- СТЕК
  ...
  - перенные примитивов хранятся в стеке
...
### Приведение в арифмитических выржаниях
- если в double сохранить int1 / int2 - будет х.0, дробная часть будет 0!!
  - если хотябы один операнд будет double, будет атвоматическое приведение к double деления(тоже самое при сложение, вычитани и т.д.) 
